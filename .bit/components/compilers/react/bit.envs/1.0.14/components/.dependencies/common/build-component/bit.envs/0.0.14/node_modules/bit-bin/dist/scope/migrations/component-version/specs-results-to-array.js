 ...
 */
function specsResultstoArray(versionModel) {
  ...
  if (versionModel.specsResults && !Array.isArray(versionModel.specsResults)) {
    ...
    const specFile = versionModel.files.filter(file => {
      return file.test;
    })[0]; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

    const oldSpecsResults = versionModel.specsResults; // Add the spec file path

    oldSpecsResults.specFile = specFile.relativePath;
    const specsResults = [oldSpecsResults]; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
