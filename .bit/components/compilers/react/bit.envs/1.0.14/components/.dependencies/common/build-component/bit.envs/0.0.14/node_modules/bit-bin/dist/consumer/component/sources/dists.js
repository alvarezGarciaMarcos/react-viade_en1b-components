...
class Dists {
  ...
  static getDistDirWhenDistIsOutsideCompDir(workspaceConfig, componentRootDir) {
    ...
    const distTarget = workspaceConfig.distTarget || _constants().DEFAULT_DIST_DIRNAME;
  ...
  calculateDistFileForAuthored(componentFile, consumer, isMain) {
    ...
    const distTarget = consumer.config.distTarget || _constants().DEFAULT_DIST_DIRNAME;

    return path().join(distTarget, distFile);
  }

  toDistFilesModel(consumer, originallySharedDir, compiler) {
    ...
    const addSharedDirAndDistEntry = pathStr => {
      ...
      const withDistEntry = this.distEntryShouldBeStripped ? path().join(consumer.config.distEntry, withSharedDir) : withSharedDir;
