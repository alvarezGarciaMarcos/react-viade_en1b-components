...
class BitMap {
  ...
  static load(dirPath) {
    ...
    try {
      componentsJson = _commentJson().default.parse(mapFileContent.toString('utf8'), null, true);
    } catch (e) {
  ...
  getConfigDirsAndFilesToIgnore(consumerPath, workspaceConfig) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      const populateIgnoreListP = _ramda().default.values(_this.components).map(
      /*#__PURE__*/
      function () {
  ...
  getAllBitIds(origin) {
    ...
    const getIdsOfOrigin = oneOrigin => {
      ...
      const allComponents = _ramda().default.values(this.components);
  ...
  validateConfigDir(compId, configDir) {
    ...
    const comps = _ramda().default.pickBy(component => {
    ...
    if (!_ramda().default.isEmpty(comps)) {
      const id = _ramda().default.keys(comps)[0];

      const stringId = _bitId().BitId.parse(id).toStringWithoutVersion();
  ...
  getExistingBitId(id, shouldThrow = true) {
    ...
    const components = _ramda().default.values(this.components);
    ...
    const componentWithScope = components.find(componentMap => {
    ...
    const componentWithoutScope = components.find(componentMap => {
  ...
  addComponent({
    ...
    if (this.components[componentIdStr]) {
      ...
      const componentMap = new (_componentMap().default)({
        files,
        origin
      });
      componentMap.setMarkAsChangedCb(this.markAsChangedBinded);
    ...
    if (rootDir) {
      this.components[componentIdStr].rootDir = (0, _utils().pathNormalizeToLinux)(rootDir);
    }
    ...
    if (trackDir) {
      this.components[componentIdStr].trackDir = (0, _utils().pathNormalizeToLinux)(trackDir);
    }
  ...
  getComponentObjectOfFileByPath(componentPath) {
    const components = this.getAllComponents();
    return _ramda().default.pickBy(component => (0, _utils().pathIsInside)(componentPath, component.rootDir || this.projectRoot), components);
  }
  /**
  ...
  updatePathLocation(from, to, existingPath) {
    ...
    const allChanges = [];
    Object.keys(this.components).forEach(componentId => {
      const componentMap = this.components[componentId];
