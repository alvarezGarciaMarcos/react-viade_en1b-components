...
const mock = require('mock-fs'); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!


const path = require('path'); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
...
const mockRootDir = path.join(__dirname, '..', '..', '..'); // needed for the lazy loading
...
describe('filing-cabinet', () => {
  describe('JavaScript', () => {
    ...
    it('dangles off its supported file extensions', () => {
      assert.deepEqual(cabinet.supportedFileExtensions, ['.js', '.jsx', '.ts', '.tsx', '.scss', '.sass', '.styl', '.less', '.vue']);
    });
    ...
    describe('commonjs', () => {
      ...
      it('adds the directory to the require resolution paths', () => {
        ...
        assert.ok( // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
      ...
      it.skip('resolves a node module with module entry in package.json', () => {
        ...
        assert.equal(result, path.join(path.resolve(directory), '..', 'node_modules', 'module.entry', 'index.module.js'));
      });
  ...
  describe('.scss with a dependency prefix with a tilda and resolve config', () => {
    describe('when the alias in resolve-config is resolved to an existing file', () => {
      it('should resolve the dependency according to the resolve-config', () => {
        const resolveConfig = {
          aliases: {
            '~bootstrap': path.normalize(fixtures)
          }
        };
        const result = cabinet({
          resolveConfig,
